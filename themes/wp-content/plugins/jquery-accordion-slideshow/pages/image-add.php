...
if (isset($_POST['JaS_form_submit']) && $_POST['JaS_form_submit'] == 'yes')
{
	...
	if($JaS_pause != "true" && $JaS_pause != "false")
	{
		$JaS_pause = "true";
	} 
	
	if($JaS_Random != "YES" && $JaS_Random != "NO")
	{
		$JaS_Random = "YES";
	} 
	
	if($JaS_trigger != "click" && $JaS_trigger != "mouseover")
	{
		$JaS_trigger = "click";
	}
	
	if($JaS_invert != "true" && $JaS_invert != "false")
	{
		$JaS_trigger = "true";
	} 
...
<div class="form-wrap">
	...
	<form name="JaS_form" method="post" action="#" onsubmit="return JaS_submit()"  >
      <h3><?php _e('Add details', 'jquery-accordion-slideshow'); ?></h3>
		...
		<p><?php _e('Where is the picture located on your server? Put the address here.', 'jquery-accordion-slideshow'); ?><br />Example: wp-content/plugins/jquery-accordion-slideshow/gallery1/</p>
